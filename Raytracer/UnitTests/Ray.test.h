...
class RayTestSuite : public CxxTest::TestSuite
  {
  public:
    void testRayDefaultConstr(void)
      {
      Ray r;
    ...
    void testRayConstrWithCoords(void)
      {
    ...
    void testRayDifferentialDefaultConstr(void)
      {
      RayDifferential rd;
      TS_ASSERT(rd.m_has_differentials==false);
      }

    void testRayAndRayDifferentialMutualConstructors(void)
      {
      Point3D_d origin(1.1,2.1,-3.1);
      Vector3D_d direction(-1.5,0.0,3.1);
      Ray r(origin, direction, -100.0, -10.0);
