...
AreaLightSource::AreaLightSource(const Spectrum_d &i_radiance, intrusive_ptr<TriangleMesh> ip_mesh):
...
Spectrum_d AreaLightSource::Radiance(const Vector3D_d &i_light_direction, const Vector3D_d &i_light_normal) const
  {
  ASSERT(i_light_direction.IsNormalized());
  ASSERT(i_light_normal.IsNormalized());

  if (i_light_direction*i_light_normal > 0.0)
    return m_radiance;
  else
    return Spectrum_d(0.0);
  }

Spectrum_d AreaLightSource::Power() const
  {
...
double AreaLightSource::LightingPDF(const Vector3D_d &i_lighting_vector, size_t i_triangle_index) const
  {
...
Spectrum_d AreaLightSource::SamplePhoton(double i_triangle_sample, const Point2D_d &i_position_sample, const Point2D_d &i_direction_sample, Ray &o_photon_ray, double &o_pdf) const
  {
...
void AreaLightSource::_SampleArea(double i_triangle_sample, const Point2D_d &i_sample, Point3D_d &o_sampled_point, Vector3D_d &o_geometric_normal) const
  {
