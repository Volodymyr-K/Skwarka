...
m_use_shading_normals(i_use_shading_normals)
  ...
  m_incident_triangles_index.assign(m_vertices.size()+1,0);
  for(size_t i=1;i<m_vertices.size()+1;++i)
    m_incident_triangles_index[i]=m_incident_triangles_index[i-1]+number_of_incident_triangles[i-1];

  m_incident_triangles.assign(m_incident_triangles_index.back(),0);
  for(size_t i=0;i<m_triangles.size();++i)
    {
    const MeshTriangle &triangle = m_triangles[i];
    m_incident_triangles[m_incident_triangles_index[triangle.m_vertices[0]]++]=i;
    m_incident_triangles[m_incident_triangles_index[triangle.m_vertices[1]]++]=i;
    m_incident_triangles[m_incident_triangles_index[triangle.m_vertices[2]]++]=i;
    }

  m_incident_triangles_index[0]=0;
  for(size_t i=1;i<m_vertices.size();++i)
    m_incident_triangles_index[i]=m_incident_triangles_index[i-1]+number_of_incident_triangles[i-1];

  _ComputeTopologyInfo();
  _ComputeShadingNormals();
  _ComputeShadingNormalsDerivatives();
  }

void TriangleMesh::_ComputeShadingNormals()
  {
  ...
  for (size_t i=0;i<m_vertices.size();++i)
    ...
    size_t number_of_incident_triangles = GetNumberOfIncidentTriangles(i);
    for (size_t j=0;j<number_of_incident_triangles;++j)
      {
      size_t triangle_index = GetIncidentTriangleIndex(i,j);
...
void TriangleMesh::_ComputeShadingNormalsDerivatives()
  {
  m_dn_du.assign(m_triangles.size(),Vector3Df());
  m_dn_dv.assign(m_triangles.size(),Vector3Df());

  for (size_t i=0;i<m_triangles.size();++i)
    {
    MeshTriangle triangle = GetTriangle(i);

    float uvs[3][2];
    _GetUVs(triangle,uvs);

    // Compute shading normal and derivatives
    Vector3Df vertex_normals[3] = {
      m_shading_normals[triangle.m_vertices[0]],
      m_shading_normals[triangle.m_vertices[1]],
      m_shading_normals[triangle.m_vertices[2]]};
    ...
    if (determinant == 0.0)
      ...
      m_dn_du[i] = ( dv2 * (vertex_normals[1] - vertex_normals[0]) - dv1 * (vertex_normals[2] - vertex_normals[0])) * invdet;
      m_dn_dv[i] = (-du2 * (vertex_normals[1] - vertex_normals[0]) + du1 * (vertex_normals[2] - vertex_normals[0])) * invdet;
      }
    }
...
void TriangleMesh::ComputeDifferentialGeometry(size_t i_triangle_index, const RayDifferential &i_ray, DifferentialGeometry &o_dg) const
  ...
  if (m_use_shading_normals)
    ...
    Vector3Dd vertex_normals[3] = {
      ...
      o_dg.m_dn_du = Convert<double>(m_dn_du[i_triangle_index]);
      o_dg.m_dn_dv = Convert<double>(m_dn_dv[i_triangle_index]);
    }
  else
    {
...
void TriangleMesh::GetIncidentTriangles(size_t i_vertex_index1, size_t i_vertex_index2, std::vector<size_t> &o_triangles) const
  {
  ASSERT(i_vertex_index1 < m_vertices.size());
  ASSERT(i_vertex_index2 < m_vertices.size());
...
void TriangleMesh::_ComputeTopologyInfo()
  {
    ...
    if (visited[i]==false)
      ...
      while(qu.empty()==false)
        ...
        for(unsigned char v=0;v<3;++v)
          ...
          GetIncidentTriangles(v1,v2,triangles);
