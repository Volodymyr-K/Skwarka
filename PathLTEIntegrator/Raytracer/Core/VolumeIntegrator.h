...
* A volume integrator is an algorithm for evaluation of the light transport equation taking into account only volume scattering (i.e. no primitives).
* @sa LTEIntegrator
*/
class VolumeIntegrator: public ReferenceCounted
  ...
  public:
    /**
    * Computes radiance and media transmittance along the specified ray.
    * @param i_ray Ray for which the radiance and transmittance is to be computed. The direction component of the ray should be normalized.
    ...
    */
    virtual Spectrum_d RadianceAndTransmittance(const RayDifferential &i_ray, const Sample *ip_sample, Spectrum_d &o_transmittance) const = 0;
    ...
    * This method should be called before the RadianceAndTransmittance() and Transmittance() methods are called.
