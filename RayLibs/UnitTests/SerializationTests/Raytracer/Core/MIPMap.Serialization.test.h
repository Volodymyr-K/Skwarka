...
class MIPMapSerializationTestSuite : public CxxTest::TestSuite
  {
  public:
    void test_MIPMap_Serialization()
      {
      intrusive_ptr<MIPMap<Spectrum_d> > p_map1 ( _CreateBlueRedMIPMap(123,234,true,8.0) );
      ...
      for(double u=0;u<=1.0;u+=0.1)
        for(double v=0;v<=1.0;v+=0.1)
          {
          Spectrum_d t1 = p_map1->Evaluate(Point2D_d(u,v), Vector2D_d(0.02,0.0), Vector2D_d(0.0,0.2));
          Spectrum_d t2 = p_map2->Evaluate(Point2D_d(u,v), Vector2D_d(0.02,0.0), Vector2D_d(0.0,0.2));
          if (t1!=t2)
            {
