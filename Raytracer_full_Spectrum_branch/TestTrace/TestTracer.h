...
#include <Raytracer/Core/SpectralConversions.h>
...
inline void TestTracer::LoadMesh()
  {
  RGBColor_d col1(1.0, 0.0, 1.0);
  Spectrum2_d sp = RGBToSpectrum(col1);
  //for(int i=0;i<SpectralSamplesNum;++i) sp[i]=1.0;
  RGBColor_d col2 = SpectrumToRGB(sp);

  RGBColor_d col1_1(0.0, 1.0, 0.0);
  SpectralCoef_d sp_1 = RGBToSpectralCoef(col1_1);
  RGBColor_d col2_1 = SpectralCoefToRGB(sp_1);

  sp *= sp_1;
  RGBColor_d col3 = SpectrumToRGB(sp);

  for(int i=0;i<SpectralSamplesNum;++i)
    printf("%lf\n",sp[i]);
